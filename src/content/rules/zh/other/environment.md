...
角色可以与灌木丛处于同一位置，处于灌木丛时，这些灌木丛可以为目标提供少量掩蔽，并且隐藏（躲藏）检定获得5DP环境加值，而隐藏（潜行）和运动相关检定获得5DP环境减值，但是移动时需要耗费四倍移动力（而非一般困难地形的二倍）。
...
强光环境：强光环境下人物会持续受到不可叠加的目眩点数，每轮都要豁免一次。这种目眩点数每轮刷新而不会持续存在，只要没有因此陷入毁灭性后果，它会在离开强光环境后以每轮一点的速度消退。

强光环境带来的目眩点数和光照强度有关。亮度的单位为尼特（nits），也称做堪德拉每平米（cd/m2）

黑暗环境：环境亮度小于1尼特

昏暗环境：环境亮度在1-200尼特

明亮环境：环境亮度200-10000尼特

强光环境：环境亮度高于10000尼特，此时目眩点数为1

环境亮度每上升十倍，造成的目眩点数多出一点。

环境亮度不意味着其中所有物体的亮度都和环境亮度在一个区间。
