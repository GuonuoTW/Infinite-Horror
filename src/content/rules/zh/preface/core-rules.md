...
无限恐怖桌面扮演游戏采用10面骰（以后统称为D10，即die 10）与成功数判定规则。当你的人物需要做些什么事、并且在一次尝试中成功或失败会带来不同后果时，你应该为他进行一次**检定**，这通常包括了以下几个步骤：
...
特殊：人物在一个攻击动作中攻击一个目标多次时，若在攻击的过程获得攻击附加成功（命中，根据检定获得等），则人物可以立即使用这些攻击附加成功，则一旦触发一次后，后续本动作内不会再次触发。

特殊：若使用多个武器针对同一个目标进行多次攻击，或者针对多个目标进行攻击，则根据每一次使用的武器单独分配攻击附加成功（例如你在一次攻击动作中使用一柄普通的长剑与一柄攻击时+4附加成功巨斧分别进行攻击，则巨斧带来的4点附加成功只能在巨斧本身进行的那些攻击上分配，不能挪到长剑的攻击上）。
...
防御附加成功必须在一次攻击投掷之前宣言使用的数量。因此有可能会出现防御方宣言了5个附加成功，而攻击方只投掷了1个成功数的局面。这种情况下，防御方有4个附加成功**无可避免地浪费了**。

也有可能防御方忘记（或者其他理由）在攻击投掷前宣言使用多少防御附加成功，这种情况下，视为没有使用防御附加成功。

但如果ST倾向于更加轻松的跑团，也可以允许玩家们在作为防御方时，在受到一次攻击之后根据攻击方的成功数再为这次攻击自由分配防御附加成功。
...
**多次乘****除法运算与加法**  
有时候规则让你在一项数值上乘一个数（或数倍于一个数）或一个投出的值或加上一个数。当只有一个乘数或加数时，正常地乘或加那个数即可。而当有两个或多个乘数乘以任何抽象值（比如修正或投出的值，以及成功数等）或同时有乘数和加数存在时，将所有的乘数带来的影响视为加上那个乘数-1的倍数，加数最后计入。

例：一个因血统获得2倍陆行移动速度的人物使用了一个暂时将陆行速度变为4倍的能力，又穿了一双令陆行移动速度增加50的鞋时，他此时实际的陆行移动速度视为提升1倍再提升3倍再增加50，即陆行速度\*（1+1+3）+50。

请注意，上述规则是针对单一的一项数值进行的运算，对于一个数值的衍生数值，在该数值计算完毕之后再进行衍生数值的计算，而不是将两者的运算混在一起。

例：一个角色的基础移动速度翻倍且增加10，然后其陆行速度变为原来的四倍，他此时实际陆行移动速度为（基础速度\*2+10）\*4.

**除法运算：**

有的规则让你获得某些将XX减半的能力，如消耗，休息的时间等。若你拥有多个，则直接全部如常先算乘除，再计算加减。

例：你因血统获得魔力消耗减半的效果，然后你再使用了使魔力消耗减半的能力，然后又拿了一个降低1点魔力消耗的武器。他此时实际的消耗魔力点数是基础魔力的四分之一，然后再减少1点。

这些运算不适用于玩家自身数值以外的东西，对于金钱、距离等内容，如常使用四则运算。
